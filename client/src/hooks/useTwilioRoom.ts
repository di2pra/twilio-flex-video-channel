...
const useTwilioRoom = () => {
  ...
  const [customerParticipant, setCustomerParticipant] = useState<LocalParticipant>();
    ...
    if (customerParticipant) {
      ...
      Array.from(customerParticipant.tracks.values()).forEach(track => {
        if (track.track && track.track.kind === "video") {
          // @ts-ignore
          setCustomerTracks(prev => { return { ...prev, video: track.track } });
        }

        if (track.track && track.track.kind === "audio") {
          // @ts-ignore
          setCustomerTracks(prev => { return { ...prev, audio: track.track } });
        }
      });
