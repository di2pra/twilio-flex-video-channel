...
const VideoMonitor = ({ manager, ...props }: Props) => {
  ...
  const { getTokenSupervisor } = useApi({ token: manager.store.getState().flex.session.ssoTokenPayload.token });

  const { agentTracks, customerTracks, partnerAudioTrack, connectRoom, disconnectRoom } = useTwilioRoomSupervisor();
  ...
  useEffect(() => {
    ...
    if (customerTracks.video) {
      ...
      if (customerMediaContainer.current) customerMediaContainer.current.style.display = 'block';
      customerMediaContainer.current?.appendChild(customerTracks.video.attach());

    } else {

      if (customerMediaContainer.current) customerMediaContainer.current.style.display = 'none';
  ...
  useEffect(() => {
    ...
    if (customerTracks.audio) {
      customerMediaContainer.current?.appendChild(customerTracks.audio.attach());
    } else {
  ...
  useEffect(() => {
    ...
    if (partnerAudioTrack) {
      ...
      if (partnerAudioContainer.current) partnerAudioContainer.current.style.display = 'block';
      partnerAudioContainer.current?.appendChild(partnerAudioTrack.attach());

    } else {

      if (partnerAudioContainer.current) partnerAudioContainer.current.style.display = 'none';
